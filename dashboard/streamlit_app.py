...
df = pd.read_csv("https://raw.githubusercontent.com/andinnnputrii/data-lifecycle-smart-farming-23082010056/main/outputs/cleaned_data.csv")
df["timestamp"] = pd.to_datetime(df["timestamp"])

col1, col2, col3, col4 = st.columns(4)
col1.metric("Total Farms", df["farm_id"].nunique())
col2.metric("Avg Soil Moisture", str(round(df["soil_moisture_%"].mean(), 1)) + "%")
col3.metric("Avg Yield", str(round(df["yield_kg_per_hectare"].mean(), 0)) + " kg/ha")
col4.metric("Avg Temperature", str(round(df["temperature_C"].mean(), 1)) + "C")
...
ax1.plot(df_sorted["timestamp"], df_sorted["soil_moisture_%"], color="#2196F3", linewidth=0.8, alpha=0.7)
...
with col_left:
    st.subheader("Gauge - Rata-rata Soil Moisture")
    ...
    ax2.axhline(y=threshold, color="red", linestyle="--", linewidth=2, label="Threshold 20%")
    ax2.set_ylim(0, 50)
    ax2.set_ylabel("Soil Moisture (%)")
    ax2.legend()
    ax2.text(0, avg_moisture + 1, str(round(avg_moisture, 1)) + "%", ha="center", fontsize=14, fontweight="bold")
    st.pyplot(fig2)

with col_right:
    st.subheader("Heatmap - Korelasi Antar Sensor")
    sensor_cols = ["soil_moisture_%", "soil_pH", "temperature_C", "rainfall_mm", "humidity_%", "sunlight_hours", "NDVI_index", "yield_kg_per_hectare"]
    corr_matrix = df[sensor_cols].corr()
    fig3, ax3 = plt.subplots(figsize=(6, 5))
    sns.heatmap(corr_matrix, annot=True, fmt=".2f", cmap="coolwarm", ax=ax3, square=True, linewidths=0.5)
    st.pyplot(fig3)
...
ax4.axvline(x=yield_threshold, color="red", linestyle="--", linewidth=2, label="Threshold 4000 kg/ha")
...
q3.metric("Timeliness", str(timeliness) + "%")
